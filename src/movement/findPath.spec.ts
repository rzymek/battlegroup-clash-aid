...
describe('buildTerrainGrid', () => {
  it('assigns default cost to cells outside polygons', () => {
    const grid = buildTerrainGrid(SVG_WITH_POLYGON, { forest: 3 }, 1, 10, 10);
...
describe('findPath', () => {
  it('finds a straight path when no terrain obstacle exists', () => {
    const grid = buildTerrainGrid(SVG_WITH_POLYGON, {}, 1, 10, 10);
  ...
  it('assigns higher cost to path crossing expensive terrain', () => {
    const grid = buildTerrainGrid(SVG_WITH_POLYGON, { forest: 10 }, 1, 20, 20);
